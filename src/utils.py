...
def embed(chunk_dir, index_dir, model_name, **kwarg):
    ...
    if "contriever" in model_name:
        model = SentenceTransformer(model_name, device="cuda:1" if torch.cuda.is_available() else "cpu")
    else:
        model = CustomizeSentenceTransformer(model_name, device="cuda:1" if torch.cuda.is_available() else "cpu")
...
class Retriever: 
    ...
    def get_relevant_documents(self, question, k=32, id_only=False, **kwarg):
        ...
        if "bm25" in self.retriever_name.lower():
            ...
            with torch.no_grad():
                query_embed = self.embedding_function.encode(question, batch_size=16, show_progress_bar=True,**kwarg)
...
class RetrievalSystem:
    ...
    def retrieve(self, question, k=32, rrf_k=100, id_only=False):
        ...
        try:
            ...
            # print("texts before extract", texts)

            print("error in the util retrieval", e)
        return texts, scores
    ...
    def extract(self, ids):
        ...
        if self.cache:
            try:
                ...
                print("id", id)
                print("error in the util and extract in the doc", e)
        else:
            output = []
            for i in ids:
                item = self.dict[i] if type(i) == str else self.dict[i["id"]]
